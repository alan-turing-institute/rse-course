...
# We will demonstrate an algorithm to simulate flocking behaviour in numpy.
# The simulation consists of a set of individual bird-like objects that we will call 'boids' following the nomenclature of [the original paper](http://www.cs.toronto.edu/~dt/siggraph97-course/cwr87/) for more details.
...
# We used **broadcasting** with np.newaxis to apply our upper limit to each boid. `rand` gives us a random number between 0 and 1.
# We multiply by our limits to get a number up to that limit.
...
# For example, let's assume that we want our initial positions to vary between 100 and 200 in the x axis, and 900 and 1100 in the y axis.
...
# But each boid will also need a starting velocity.
# Let's make these random too:
# 
# We can reuse the `new_flock` function defined above, since we're again essentially just generating random numbers from given limits.
# This saves us some code, but keep in mind that using a function for something other than what its name indicates can become confusing!
# 
...
# So now we can animate our Boids using the matplotlib animation tools
# All we have to do is import the relevant libraries:
...
# The examples given below are examples of how to use numpy to efficiently apply equations to arrays.
# here are many potential ways to do such things, and this is intended as a showcase of numpy's versatility rather than a prescribed set of rules.
...
# We'll need a matrix giving the distances between each boid.
# This should be $N \times N$.
