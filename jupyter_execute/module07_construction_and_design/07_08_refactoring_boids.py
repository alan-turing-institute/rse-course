...
# 
# There's not a single "right" answer to how the code should be refacatored, but on the `better_boids` branch of the `bad-boids` repo we have an improved version of the code with changes based on all the ideas above. You can find it on GitHub here (but try not to look before making your own changes):
# 
